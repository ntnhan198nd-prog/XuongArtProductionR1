...
const Footer = ({ content, social, cta, minimal = false, showContact = false }) => {
  ...
  // Dark contact band — picked up the address / phone / email block that
  // used to live inside the homepage's CTA banner. The CTA itself is now
  // limited to /whoweare; the contact info follows the user down to the
  // footer area on pages that opt in (currently just the homepage).
  const contactBand = showContact ? (
